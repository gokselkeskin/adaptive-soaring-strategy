...
for individual in individuals:
    ...
    for day in days:
        ...
        for idx in range(0, len(df['heading_thermal'])):
            ...
            if math.isnan(heading_value):
                ...
                if counter_number >= threshold_value:
                    ...
                    # if len(higherPeaks_x) or len(higherPeaks_y) or len(lowerPeaks_y) or len(lowerPeaks_x) < 3:
                    #    break
                    # if abs(len(lowerPeaks_y) - len(lowerPeaks_x)) > 8:
                    #    continue
                    ...
                    # differencePeaks_x = (higherPeaks_x + lowerPeaks_x) / 2
                    differencePeaks_x1 = (higherPeaks_x + lowerPeaks_x) / 2
                    # differencePeaks_x2 = (higherPeaks_x + (lowerPeaks_x[+1])) / 2
                    # differencePeaks_x = np.concatenate((differencePeaks_x1, differencePeaks_x2))
                    # differencePeaks_x = np.unique(differencePeaks_x, axis=0)
                    ...
                    # differencePeaks_y = (higherPeaks_y + lowerPeaks_y) / 2
                    differencePeaks_y1 = (higherPeaks_y + lowerPeaks_y) / 2
                    # differencePeaks_y2 = (higherPeaks_y + (lowerPeaks_y[+1])) / 2
                    # differencePeaks_y = np.concatenate((differencePeaks_y1, differencePeaks_y2))
                    # differencePeaks_y = np.unique(differencePeaks_y, axis=0)
                    ...
                    if len(range_mid_y) == 0 or len(range_mid_x) == 0:
                        ...
                        extract_series = pd.DataFrame()
                        continue
                    # if range_mid_x or range_mid_y
                    # range_full = np.arange(int(min(extract_series.index)), int(max(extract_series.index)))
                    ...
                    extract_series["y_update_MA"] = extract_series["y_update"].rolling(5, center=True,
                                                                                       win_type='gaussian').mean(std=1)
                    # extract_series["x_update_MA"] = extract_series["x_update"]
                    # extract_series["y_update_MA"] = extract_series["y_update"]
                    ...
                    # print(np.mean(extract_series["R_update"][min(shared_mid):max(shared_mid)]))
                    # print(np.mean(extract_series["Vxy_update"][min(shared_mid):max(shared_mid)]))
        ...
        # df["Xw","Yw"] = df["Unnamed: 0"].map(new_dataframe("Unnamed: 0")["Xw","Yw"])

        # cols = ['Unnamed: 0','Unnamed: 0.1', 'Species', 'date', 'lat', 'lon', 'ele', 'individuals',
        #       'CoordinatesX(m)', 'CoordinatesY(m)', 'only_date', 'date_in_unix',
        #       'gap', 'time_delta', 'to_impute', 'to_discard', 'Vx', 'Vy', 'Vz', 'Vxy',
        #       'Ax', 'Ay', 'K_Raw', 'sampling_rate', 'X(m)_MA', 'Y(m)_MA', 'Z(m)_MA',
        #       'Vx_MA', 'Vy_MA', 'Vxy_MA', 'Vz_MA', 'Ax_MA', 'Ay_MA', 'K_MA',
        #       'horizontal_speed', 'vertical_speed', 'K', 'heading', 'R']
